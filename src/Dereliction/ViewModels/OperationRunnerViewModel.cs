                    ...
                    foreach ((string fakeRoot, string fake) input in inputModel.Inputs)
                    {
                        ...
                        foreach (var processor in processors)
                        {
                            ...
                            foreach (var data in Coordinator.RunSegmented(processor.processor, input, src, 0))
                            {
                                ...
                                if (DirectOutput)
                                {
                                    if (!data.Dry && data.DefaultFormat != Data.ExportUnsupported)
                                    {
                                        try
                                        {
                                            using var d = data;
                                            string bp = processor.processor.GenPath(d.GetExtension(),
                                ...
                                else
                                {
                                    results.Add(data);
                                    Outputs.Add(
                                        new DataFsElement(Path.Combine(input.fakeRoot, data.BasePath), data));
                                }
                            }
